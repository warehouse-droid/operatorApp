...
UPDATE sales_orders s
   SET status_updated_at = d.status_updated_at
  FROM delivery_orders d
 WHERE d.netsuite_id = s.netsuite_id
   AND d.order_type = 'sales_order'
   AND s.status_updated_at IS NULL;
...
UPDATE transfer_orders t
 ...
 WHERE d.netsuite_id = t.netsuite_id
   ...
   AND t.status_updated_at IS NULL;

UPDATE purchase_orders
   SET status_updated_at = COALESCE(received_at, synced_at)
...
BEGIN
  ...
  INSERT INTO transfer_orders (
    ...
    netsuite_missing_at, status_updated_at
  )
  SELECT
