...
export function loadEndYard(load = {}, ownYards = DEFAULT_OWN_YARDS, plan = {}) {
  const own = new Set((ownYards || []).map(text));
  if (load.returnOnly) {
...
export function validateDispatchLoadAssignments(plan = {}, {
  ...
  const own = new Set((ownYards || DEFAULT_OWN_YARDS).map(text));
  ...
  for (const [driverLogin, driverRows] of byDriver.entries()) {
    ...
    for (let index = 1; index < driverRows.length; index += 1) {
      ...
      const previousEndYard = loadEndYard(previous.load, ownYards, plan);
      ...
      const targetAvailableYard = previousTargetUse
        ? loadEndYard(previousTargetUse.load, ownYards, plan)
...
export function changedLockedLoadAssignments(previousPlan = {}, nextPlan = {}, lockedLoadIds = new Set()) {
  ...
  for (const loadId of lockedLoadIds || []) {
    ...
    const previousSignature = previous ? JSON.stringify({
      driverLogin: previous.driverLogin,
      truckPlate: previous.truckPlate,
      stopIds: (previous.load.stops || []).map((stop) => text(stop.id)),
      orderRefs: (previous.load.stops || []).map((stop) => text(stop.orderId)).filter(Boolean),
      allocations: allocationSignature(previousPlan, previous)
    }) : "";
    const currentSignature = current ? JSON.stringify({
      driverLogin: current.driverLogin,
      truckPlate: current.truckPlate,
      stopIds: (current.load.stops || []).map((stop) => text(stop.id)),
      orderRefs: (current.load.stops || []).map((stop) => text(stop.orderId)).filter(Boolean),
      allocations: allocationSignature(nextPlan, current)
    }) : "";
