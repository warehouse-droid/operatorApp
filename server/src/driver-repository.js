...
import { dispatchLoadAssignment, flattenDispatchPlanLoads, normalizeDispatchPlanLoadAssignments } from "./dispatch-load-assignment.js";
...
function loadEndOwnYard(plan, load) {
  ...
  if (load.returnOnly) {
    ...
    return OWN_YARD_CODES.has(yard) ? yard : "";
  }
  const stop = lastRoutedStop(load);
  if (!stop) return "";
  if (stop.type === "pick") {
  ...
  return candidates.find((value) => OWN_YARD_CODES.has(value)) || "";
}
