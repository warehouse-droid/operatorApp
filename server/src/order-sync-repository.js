...
export async function upsertSalesOrders(orders = []) {
  for (const order of orders || []) {
    ...
    await enqueueNetSuiteMirrorOrderEvent("sales_order", normalized.netsuite_id);
  }
}
    ...
    await query(
      `INSERT INTO transfer_orders (
       ...
       ON CONFLICT (netsuite_id) DO UPDATE SET
         ...
         status = EXCLUDED.status,
         status_text = EXCLUDED.status_text,
         from_location_id = COALESCE(EXCLUDED.from_location_id, transfer_orders.from_location_id),
         from_location = COALESCE(NULLIF(BTRIM(EXCLUDED.from_location), ''), transfer_orders.from_location),
         to_location_id = COALESCE(EXCLUDED.to_location_id, transfer_orders.to_location_id),
         to_location = COALESCE(NULLIF(BTRIM(EXCLUDED.to_location), ''), transfer_orders.to_location),
         memo = EXCLUDED.memo,
         expected_delivery_date = EXCLUDED.expected_delivery_date,
         dispatch_address = EXCLUDED.dispatch_address,
         dispatch_window_start = EXCLUDED.dispatch_window_start,
...
export async function upsertSalesOrderLines(orderId, lines = []) {
  ...
  await enqueueNetSuiteMirrorOrderEvent("sales_order", orderId);
}
...
async function upsertTransferOrderLines(orderId, lines = [], stage) {
  ...
  await enqueueNetSuiteMirrorOrderEvent("transfer_order", orderId);
}
...
export async function markOutboundOrderMissing(orderId, { orderFamily = "sales_order" } = {}) {
  ...
  await enqueueNetSuiteMirrorOrderEvent("sales_order", orderId, { changeType: "missing" });
}
...
export async function updateSalesOrderNetSuiteStatus(orderId, patch = {}) {
  ...
  if (updated) await enqueueNetSuiteMirrorOrderEvent("sales_order", orderId, { changeType: "status" });
  return updated;
}
...
export async function markMissingInboundOrders({ orderFamily, activeOrderIds = [], sourceLocationId = null, destinationLocationId = null } = {}) {
  ...
  const missingOrders = await query(
    `UPDATE transfer_orders
        SET netsuite_active = false,
            netsuite_missing_at = now(),
            synced_at = now()
