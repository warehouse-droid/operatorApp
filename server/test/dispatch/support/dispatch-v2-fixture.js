...
export async function createDispatchV2Fixture() {
   ...
   * This intentionally bypasses legacy save side effects; v2 commands are the
   * system under test and must process this normal persisted snapshot shape.
   *
  ...
  async function seedPlan({ date, refs }) {
    ...
    await query(
      `UPDATE dispatch_plan_snapshots
        ...
        WHERE plan_id = $1`,
      [plan.id, JSON.stringify(orders), JSON.stringify(dispatchTrucks(refs)), JSON.stringify({ testOnly: true })]
    );
    return {
