...
const RECOVERY_VERSION = "20260805-online-mode-v3";
...
test("the installed Driver shell cannot mix old DB/route code with photo recovery code", () => {
  ...
  for (const asset of changedAssets) {
    const versionedPath = `/${asset}?v=${RECOVERY_VERSION}`;
  ...
  assert.match(worker, /DRIVER_CACHE_NAME = `\$\{DRIVER_CACHE_PREFIX\}v19`/u);
});
...
test("Dispatch receives the photo-failure diagnostics renderer without a stale asset", () => {
  ...
  assert.ok(html.includes(`/dispatch-offline-review.css?v=${RECOVERY_VERSION}`));
  assert.ok(html.includes(`/dispatch-offline-review.js?v=${RECOVERY_VERSION}`));
});
